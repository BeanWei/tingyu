...
func GetReactionsForOneSubject(ctx context.Context, userId int64, subjectType userreaction.SubjectType, subjectId int64) ([]*types.Reaction, error) {
	reactions := make([]*types.Reaction, 0)
	if err := g.RDB().SelectContext(ctx, &reactions, `
		...
		where t1.subject_type = $2 and t1.subject_id = $3
		group by t1.subject_id, t1.react_code
	`, userId, subjectType.String(), subjectId); err != nil {
		return nil, err
	}
	return reactions, nil
}

func GetReactionsForManySubject(ctx context.Context, userId int64, subjectType userreaction.SubjectType, subjectIds []int64) (map[int64][]*types.Reaction, error) {
	reactions := make([]*types.Reaction, 0)
	query, args, err := sqlx.In(`
		...
		from user_reactions as t1
		where t1.subject_type = ? and t1.subject_id in (?)
		group by t1.subject_id, t1.react_code
	`, userId, subjectType.String(), subjectIds)
	if err != nil {
		return nil, err
	}
	...
	res := make(map[int64][]*types.Reaction)
	for _, reaction := range reactions {
		if len(res[reaction.SubjectId]) == 0 {
			res[reaction.SubjectId] = make([]*types.Reaction, 0)
		}
